...
class SharedEncoder(nn.Module):
    def __init__(self, num_encoder_heads, X_dim, embed_dim, rep_dim, device, dropout=0.7):
        super().__init__()
        self.linear1 = nn.Linear(X_dim, embed_dim)
        self.encoder = nn.TransformerEncoderLayer(d_model=embed_dim,
                                                  nhead=num_encoder_heads,
                                                  dropout=0.6,
                                                  batch_first=True)
        self.encoder2 = nn.TransformerEncoderLayer(d_model=embed_dim,
                                                   nhead=num_encoder_heads,
                                                   dropout=0.6,
                                                   batch_first=True)
        self.encoder3 = nn.TransformerEncoderLayer(d_model=embed_dim,
                                                   nhead=num_encoder_heads,
                                                   dropout=0.6,
                                                   batch_first=True)
        self.encoder4 = nn.TransformerEncoderLayer(d_model=embed_dim,
                                                   nhead=num_encoder_heads,
                                                   dropout=0.6,
                                                   batch_first=True)

        self.linear2 = nn.Linear(embed_dim, rep_dim)
        self.dropout1 = nn.Dropout(dropout)
        self.dropout2 = nn.Dropout(dropout)
        self.device = device

        self.src_position_embedding = positional_encoding2(5, embed_dim, device)
...
class Model(nn.Module):
    def __init__(self,
        ...
        self.encoder = SharedEncoder(num_encoder_heads=encoder_num_layers,
                                     X_dim=num_features,
                                     embed_dim=embed_dim,
                                     rep_dim=rep_dim,
                                     device = self.device)
        ...
        self.fc_net = FC(embed_dim=rep_dim, hidden_dim=hidden_dim, num_category=num_category)
    ...
    def get_context_vector(self, x_dx, x_rx, mask=True):
        ...
        # attention_dx: (N, num_visits, max_len, 3, 1), nvdrw
        # embedding_dx: (N, num_visits, max_len, 3, code_dim), nvdrs
        # we want context_vector: (N, num_visits, max_len, 1, code_dim), nvdws
        context_vector_dx = torch.einsum("nvdrw,nvdrs->nvdws", attention_dx, embedding_dx).squeeze(-2)
        #(N, num_visits, max_len, code_dim)

        # attention_rx: (N, num_visits, max_len, 4, 1), nvdrw
        # embedding_rx: N, num_visits, max_len, 4, code_dim), nvdrs
        # we want context_vector: (N, num_visits, max_len, 1, code_dim), nvdws
        context_vector_rx = torch.einsum("nvdrw,nvdrs->nvdws", attention_rx, embedding_rx).squeeze(-2)
        #(N, num_visits, max_len, code_dim)
    ...
    def get_compressed_visit_vector(self, x_dx, x_rx, context_vector_dx, context_vector_rx, mask=True):
        ...
        return compressed_context_vector.squeeze(-1), attentions_visit, (energies_visit_dx, energies_visit_rx)
    ...
    def get_compressed_incoded_visit(self, encoded_inp):
        ...
        return compressed_incoded_visit.squeeze(1), attentions_compresed_visit, compresed_visit_energies#(N,embed_dim)
    ...
    def forward(self, x_dx, x_rx, x_demo):
        ...
        encoder_input = compressed_context_vector
        ...
        out_concat = self.fc_net(compressed_incoded_visit.unsqueeze(1))  # stack referenced on subject

        #(N, num_event, num_category)


        return out_concat, compressed_incoded_visit, (gram_att, att_visit, att_compresed_visit), (e_visit, e_compresed_visit)
...
def save_checkpoint(state, filename = "checkpoints/my_checkpoint_attack_encoder.pth.tar"):
